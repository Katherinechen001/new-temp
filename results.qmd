...
### SHAP Explanation for Linear Regression

Although linear regression gives interpretable coefficients, it assumes that the relationship between the predictors and the target are additive and linear. We check consistency and provide an additional way to confirm the real relevance of every feature on the individual predictions by same SHAP (SHapley Additive exPlanations) being computed, based on the fastshap package.
```{r}
# Load library
library(fastshap)
library(ggplot2)

# Prepare training features (remove target)
X_train <- subset(train_data, select = -price)

# Define prediction wrapper for linear model
...
shap_lm <- explain(
  ...
  nsim = 100  # Increase for better estimates if needed
)

# Plot global SHAP summary
autoplot(shap_lm, X = X_train) +
  ggtitle("SHAP Summary for Linear Regression Model")

# Plot local SHAP explanation for one observation
autoplot(shap_lm, type = "contribution", row_num = 1, X = X_train) +
  ggtitle("SHAP Explanation for Observation 1")
